...
public:
	...
	void mv(char *source, char *des);
	void cp(char *source, char *des);
	void ln(char *path, char *des);
	void chmod(int num, char *filename);
	...
	void umask(int number);      //delete some right
	void cat(string filename);
	void append(string filename, string appendContent);
	void append(string filename, char *appendContent);

	int findBestDirectory(char *path);
	int findrelativeDirectory(char *path, directory *nowcur);
	int findPathDirectory(char *path);
	...
	int findDirectory2(const char *filename, directory *currdir); //finding correspond directory based on filename
	void allocateEnoughSpace(int inodeNum);                       //the most important part    to the best of my knowledge
	void releaseEnoughSpace(int inodeNum);                        //release all directory BLOCK
	int findAvailableBlock();                                     //a api to find proper BLOCK
	void releasecCorrespondingBlock(int blocknum);                //a api to delete correspondingblock
	void writeEnoughDirectory(directory *dir);
	int allocateInodeAndEmptyDirecotry(bool flag, string filename);
	directory *readEnoughDirectory(int num);
